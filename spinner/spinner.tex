...
\foreach \i in {1,...,\nminusone}{
	...
	% \def\name{$\prefix^{\angle^\circ}_{\i}$}
	\def\name{\teamfmt{\i}}
	\coordinate[precise pin={\effangle:\name}] (\prefix\i) at (\angle:\effradius);
	\pgfmathsetmacro\theta{\dtheta/2+\angle}
	\node[right,
	%draw,rounded rectangle,rounded rectangle west arc=none,
	rotate=\theta] at (\theta:\effradius) {\roundfmt{\i}};
	\ifnum\i=1
	\node[left,
	%draw,rounded rectangle,rounded rectangle east arc=none,
	rotate=\theta] at (\theta:\effradius) {\roundfmt{Round}};
	\fi
	% \draw (0:0) -- (\angle:3.5);
}
\coordinate (\prefix0);
...
\foreach \ia in {0,...,\nminusone}{
	\foreach \ib in {\ia,...,\nminusone}{
		\draw[ultra thin,gray!15] (\prefix\ia) -- (\prefix\ib);
	}
}

% \node (\prefix0) {\prefix\nteams};
% Stick the brad in here
\node[draw,ultra thin,star,star points=9,inner sep=0.5mm] (0,0) {};
...
\foreach \i in {1,...,\novertwo}{
	...
	\def\bend{\ifnum\i=1 out=300,in=240\else cross\fi}
	\def\pos {\ifnum\i=1 0.5\else 0.5\fi}
	\def\startanchor {\ifnum\i=1 .300\else\fi}
	\edef\temp{\noexpand\draw[thick,sloped,line cap=rect,\bend,shorten <=-1pt,shorten >=-1pt] (\prefix\c\startanchor)
